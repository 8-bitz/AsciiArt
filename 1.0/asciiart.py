...
def imgToAscii(imgFile):	
	...
	for y in range (0,size[1]):			#For ever row.  0,0 is the top left.	
		for x in range (0,size[0]):		#For every column
			...
			asciiArt = 	asciiArt + greyscaleCharsRev[str(pix)]		#Create the output string
...
def imagesToMovie(fileList, videoname):
	...
	video = cv2.VideoWriter("colourVid_" + videoname, 0,30,(width, height))
	...
	video = cv2.VideoWriter("grayvid_" + videoname, 0,30,(width, height),0)
	for f in fileList:
		frame = cv2.imread(f)
		video.write(frame)
	#cv2.destroyAllWindows()

...
# - add a contrast down and compare videos
...
def imgToAscii(imgFile):
	...
	for y in range (0,size[1]):			#For ever row.  0,0 is the top left.
		for x in range (0,size[0]):		#For every column
			...
			pix = brightnessAdjust(pix, brightnessAdjustment)
...
def imagesToMovie(fileList, videoname):
	...
	#video = cv2.VideoWriter("h264_" + videoname, cv2.VideoWriter_fourcc('H','2','6','4'),frameRate,(width, height))
	#for f in fileList:
	#	frame = cv2.imread(f)
	#	video.write(frame)
	#cv2.destroyAllWindows()
